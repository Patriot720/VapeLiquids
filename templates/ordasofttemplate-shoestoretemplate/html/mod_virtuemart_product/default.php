		...
		<div class="vmproduct<?php echo $params->get ('moduleclass_sfx'); ?> productdetails">
			...
			<div class="<?php echo $pwidth ?> <?php echo $float ?>">
				<div class="spacer">
					...
					echo '<div class="img_hid">' . JHTML::_ ('link', JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $product->virtuemart_product_id . '&virtuemart_category_id=' . $product->virtuemart_category_id), $image, array('title' => $product->product_name)) . "</div>";
					...
					if ($show_price) {
						...
						if (!empty($product->prices['salesPrice'])) {
							...
							//my code for second price
							if(isset($product->allPrices['0']['override']) &&
								$product->allPrices['0']['override'] == '1' 
								&& $product->allPrices['0']['product_override_price'] > '0'){
								echo $currency->createPriceDiv ('costPrice', '', $product->prices, FALSE, FALSE, 1.0, TRUE);
							}
							//end my code for second price
